...
{
    /// <summary>
    /// 主题服务：mode(跟随系统/浅色/深色) × accent(蓝/绿/粉/自定义#RRGGBB)。
    /// XAML 中统一使用 DynamicResource Theme.* 引用，切换即时生效。
    /// </summary>
    public static class ThemeService
    {
        /// <summary>读取系统当前是否深色模式（注册表）。</summary>
        ...
        public static void Apply(bool dark, string accent = "blue")
        {
            ...
            {
                try { accentColor = (Color)ColorConverter.ConvertFromString(accent); }
            ...
            else
            {
                (byte aR, byte aG, byte aB) = accent switch
                {
                    ...
                    "purple" => ((byte)0xEC, (byte)0x48, (byte)0x99), // 粉色
                    _ => ((byte)0x2F, (byte)0x80, (byte)0xED)         // 蓝色
                };
        ...
        private static int _lastOpacity = 85;

        /// <summary>
        /// 按百分比（60–100）把当前窗口/卡片背景色加上 alpha 写入
        /// Theme.WindowBgAlpha / Theme.SurfaceBgAlpha，窗口与卡片绑定后整体透明。
        /// </summary>
        ...
        /// <summary>当前强调色的 RGB 反色画刷（可用于状态点等）。</summary>
        public static Brush GetInvertedAccentBrush()
        {
            try
            {
