...
| 特性 | 说明 |
|---|---|
| 🎯 **按应用切换输出设备** | 为单个应用设置输出设备，选择即生效，音量合成器同步更新 |
| 🎤 **全局麦克风静音** | 一键静音/取消所有录音设备（设备级） |
| 🧩 **托盘快捷面板** | 单击托盘图标弹出，秒切设备、调音量、静音 |
| 🖥 **完整管理界面** | 双击托盘图标打开，管理应用、设备、快捷键、主题、配置 |
| 🔊 **单独应用音量** | 滑块实时调节当前应用音量，不影响其他应用，支持静音 |
| 📋 **设备筛选 + 改名** | 设置里勾选常用设备，快捷面板只显示勾选的；可为设备自定义名称 |
| 🕵️ **自动当前应用检测** | 自动跟随最近使用/正在使用的应用，支持三种模式，前台切换实时跟随 |
| 🚫 **禁用自动切换** | 应用页可关闭单个应用的自动切换（自动跳过、仍可手动选），图标右上角显示主题色反色点 |
| ⌨️ **全局快捷键** | 切设备、调音量、静音、全局麦克风静音、开面板，全部可自定义 |
| 🖱 **托盘滚轮调音量** | 任务栏区域滚轮调节当前应用音量，OSD 实时提示 |
| 🔔 **OSD 通知** | 操作时右上角弹出应用名+音量/设备/状态，自动淡出，跟随主题 |
| 🌐 **多语言 8 种** | 中/英/日/韩/法/德/西/俄，首次启动跟随系统 |
| 🎨 **主题系统** | 深浅色跟随系统，强调色（含 RGB 自定义）、背景透明度可调 |
