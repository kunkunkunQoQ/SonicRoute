...
> Windows 11 按应用音频快速切换工具 —— 一键把单个应用的输出设备切到耳机、音箱、显示器或虚拟设备，改完立即生效。
...
| 🕵️ **当前应用自动检测** | 跟随"最近打开 / 上次操作 / 指定应用"三种模式，自动匹配前台应用 |
| ⌨️ **全局快捷键** | 切设备、调音量、静音、全局麦克风静音、打开面板，全部可自定义 |
| 🖱 **托盘滚轮调音量** | 鼠标放到托盘图标上滚轮即可调节当前应用音量，OSD 实时提示 |
...
- **快捷面板**：小巧简洁，实时显示当前应用，内置「输出设备快捷切换」「应用音量滑块」「静音 / 全局麦克风静音」「设置入口」，点击空白自动关闭，响应迅速。
- **完整管理界面**：左侧导航（概览 / 应用 / 快捷键 / 主题 / 设置）：
  - **概览**：当前应用的输出设备、音量、静音、全局麦克风静音、快捷设备切换，一键直达
  - **应用**：列出所有有音频会话的应用，可单独操作输出设备与音量
...
- **主题模式**：跟随系统 / 浅色 / 深色
- **强调色**：蓝色 / 绿色 / 紫色预设，或通过 **RGB 滑块自定义**任意颜色
...
- 全局麦克风静音（设备级，任何应用录音都生效）
- 改键优化为内联录音（免弹窗），修复数字键格式
- 通知/OSD 显示自定义设备名，长文本自动省略
- 概览页移除刷新按钮，左侧导航透明度统一
- 托盘滚轮调音量、设备筛选/改名、主题（含 RGB）/透明度等完整功能
