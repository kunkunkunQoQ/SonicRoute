...
| 🚫 **禁用自动切换** | 应用页可为单个应用关闭"自动切换为当前应用"：前台跟随/最近使用自动跳过它，但仍可手动选择；被禁用应用图标右上角显示主题色反色小圆点 |
...
单击托盘图标弹出小巧面板：顶部自动显示**当前应用**，下方一键切常用输出设备、调音量、静音 / 全局麦克风静音，点击空白自动关闭，游戏时也能秒操作。
...
- **概览**：与快捷面板功能一致——当前应用、快捷切输出设备、音量滑块、静音 / 全局麦克风静音
- **应用**：列出所有有音频会话的应用（正在播放的自动置顶），可单独切输出设备、调音量、改名、禁用自动切换
- **快捷键**：查看与重设全部全局快捷键（内联录音，按下即生效）
- **主题**：深浅色模式、强调色（预设 + RGB 自定义）、背景透明度
...
托盘滚轮调音量、全局快捷键、切换设备时，屏幕右上角弹出轻量 **OSD 通知**（应用名 + 音量/设备/状态），约 1 秒自动淡出，外观跟随主题，应用/设备名使用自定义名称。

### ⌨️ 多种快捷键

<img src="docs/images/hotkeys.png" width="700" alt="多种快捷键">

在「快捷键」页点击「修改」按下新组合键即可重新绑定：增大/减小音量、切快捷设备、静音、全局麦克风静音、打开面板。
