...
| 想影响… | 用哪档 | 入口 / 快捷键 | 不动的东西 |
|---|---|---|---|
| 只一个应用 | 当前应用档 | 面板点设备 / `Ctrl+Alt+D` 循环 | 其他应用 + 系统默认 |
| 所有「未单独设置」的应用 | 系统默认档 | `Ctrl+Alt+O/I` | 已单独设置的应用 |
| 所有运行中应用 | 全局档 | `Ctrl+Alt+Shift+O/I` | 系统默认 |
| 游戏全屏快速切 | 当前应用档 | `Ctrl+Alt+D`（游戏成为当前应用后） | — |
| 会议静音 | 静音 | `Ctrl+Shift+M` 当前应用 / `Ctrl+Shift+N` 全局麦克风 | 设备不变 |
| 全部恢复默认 | 一键还原 | `Ctrl+Alt+Shift+R` / 音量合成器「重置」 | 含已退出应用 |
