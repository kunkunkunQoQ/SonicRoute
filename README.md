...
| 场景 | 做法 |
|---|---|
| 游戏单独走耳机，其余默认 | 打开游戏 → 面板点耳机；系统默认保持音箱 |
| 只改系统默认设备 | `Ctrl+Alt+O/I`，按应用设置的保持不变 |
| 全部应用一起切（虚拟声卡） | `Ctrl+Alt+Shift+O/I`，恢复用 `Ctrl+Alt+Shift+R` |
| 一键还原所有组合 | `Ctrl+Alt+Shift+R` 或音量合成器「重置」 |
