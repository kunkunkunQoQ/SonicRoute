...
- 🔇 **应用音量 + OSD**：按 Session 独立调音量，不动系统主音量；任务栏滚轮即调；OSD可拖拽定位 / 调大小
- 🧹 **内存优化 + 多语言主题**：后台内存占用 0.4~27MB 自动回收；9 语言、RGB 强调色、透明度
...
| 🟢 含运行环境版本 | `SonicRoute-v1.13.exe` / `.zip` | ~237MB / ~85MB | 已内置运行环境 |
